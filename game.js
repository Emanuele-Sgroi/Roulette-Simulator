...
let wheelNumbersOrdered = [0, 26, 3, 35, 12, 28, 7, 29, 18, 22, 9, 31, 14, 20, 1, 33, 16, 24, 5, 10, 23, 8, 30, 11, 36, 13, 27, 6, 34, 17, 25, 2, 21, 4, 19, 15, 32];
...
function gameOver(){
	...
	popWindow.setAttribute('id', 'pop_window');
		let popWindowSpan = document.createElement('span');
		popWindowSpan.setAttribute('class', 'pop_window-span');
		popWindowSpan.innerText = 'Your balance is: 0';
		popWindow.append(popWindowSpan);

		let popWindowBtn = document.createElement('div');
		popWindowBtn.setAttribute('class', 'pop_window-btn');
		popWindowBtn.innerText = 'Play again';	
		popWindowBtn.onclick = function(){
			window.location.reload()
		};
		popWindow.append(popWindowBtn);
	container.prepend(popWindow);
}
...
function buildWheel(){
	...
	let externalRim = document.createElement('div');
	externalRim.setAttribute('class', 'external_rim');
	wheel.append(externalRim);
...
function buildBettingBoard(){
	...
	for(i = 0; i < chipValues.length; i++){
		...
		chip.onclick = function(){
			if(cvi !== 4){
				...
				stake = parseInt(chip.childNodes[0].innerText);
			}else{
				balance = balance + currentBet;
				currentBet = 0;
				document.getElementById('bankSpan').innerText = '' + balance.toLocaleString("en-GB") + '';
	...
	bankSpan.innerText = '' + balance.toLocaleString("en-GB") + '';
...
function setBet(e, n, t, o){
	lastStake = stake;
	stake = (balance < stake)? balance : stake;
	if(stake > 0){
		...
		currentBet = currentBet + stake;
		document.getElementById('bankSpan').innerText = '' + balance.toLocaleString("en-GB") + '';
		document.getElementById('betSpan').innerText = '' + currentBet.toLocaleString("en-GB") + '';
		for(i = 0; i < bet.length; i++){
			if(bet[i].numbers == n && bet[i].type == t){
				bet[i].amt = bet[i].amt + stake;
		...
		if(!e.querySelector('.chip')){
			let chipColour = (stake < 5)? 'red' : ((stake < 10)? 'blue' : ((stake < 100)? 'orange' : 'gold'));
			let chip = document.createElement('div');
			chip.setAttribute('class', 'chip ' + chipColour);
			let chipSpan = document.createElement('span');
			chipSpan.setAttribute('class', 'chipSpan');
			chipSpan.innerText = stake;
			chip.append(chipSpan);
			e.append(chip);
		}
	}
...
function spin(){
	...
	setTimeout(function(){
		if(numbersBet.includes(winningSpin)){
			...
			for(i = 0; i < bet.length; i++){
				...
				if(numArray.includes(winningSpin)){
					balance = (balance + (bet[i].odds * bet[i].amt) + bet[i].amt);
		...
		let pnClass = (redNumbers.includes(winningSpin))? 'pnRed' : ((winningSpin == 0)? 'pnGreen' : 'pnBlack');
...
function win(winningSpin, winValue, betTotal){
	if(winValue > 0){
		let popWindow = document.createElement('div');
		popWindow.setAttribute('id', 'pop_window');
			let popWindowSpan = document.createElement('div');
			popWindowSpan.setAttribute('class', 'pop_window-span');
				let nsnumber = document.createElement('span');
				nsnumber.setAttribute('class', 'nsnumber');
				nsnumber.style.cssText = (redNumbers.includes(winningSpin))? 'color:red' : 'color:black';
				nsnumber.innerText = winningSpin;
				popWindowSpan.append(nsnumber);
				let nsTxt = document.createElement('span');
				nsTxt.innerText = ' Win';
				popWindowSpan.append(nsTxt);
				let nsWin = document.createElement('div');
				nsWin.setAttribute('class', 'nsWin');
				popWindowSpan.append(nsWin);
			popWindow.append(popWindowSpan);
		container.prepend(popWindow);
		let plusText = document.createElement('p');
			plusText.setAttribute('class', 'plus_text');
			plusText.style.color = 'green';
			plusText.innerText = `+${winValue+betTotal}`;
			container.append(plusText);
		setTimeout(function(){
			popWindow.style.cssText = 'opacity:0';
		}, 1500);
		setTimeout(function(){
			popWindow.remove();
		}, 2500);
		...
		minusText.setAttribute('class', 'minus_text');
		minusText.style.color = 'red';
		minusText.innerText =`-${betTotal}`;
		container.append(minusText);
	}
	
}

function removeBet(e, n, t, o){
	stake = (stake == 0)? 100 : stake;
	for(i = 0; i < bet.length; i++){
		if(bet[i].numbers == n && bet[i].type == t){
			if(bet[i].amt != 0){
				stake = (bet[i].amt > stake)? stake : bet[i].amt;
				bet[i].amt = bet[i].amt - stake;
				balance = balance + stake;
				currentBet = currentBet - stake;
				document.getElementById('bankSpan').innerText = '' + balance.toLocaleString("en-GB") + '';
...
function spinWheel(winningSpin){
	for(i = 0; i < wheelNumbersOrdered.length; i++){
		if(wheelNumbersOrdered[i] == winningSpin){
